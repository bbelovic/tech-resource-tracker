describe("Exercise CRUD operation on tech. resource", () => {
  ...
  it("Add and edit technology resource", () => {
    ...
    cy.get('[data-testid="debug-zone"]').invoke("text").then((zone) => cy.task("log", `Create ${zone}`));
    ...
    cy.get('[data-testid="submit-btn"]').click();
    cy.get('[data-testid="debug-zone"]').invoke("text").then((zone) => cy.task("log", `Update ${zone}`));
    cy.get('[data-testid="debug-result"]').should("be.visible").and("contain.text", "result=Updated");
